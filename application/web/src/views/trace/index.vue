  ...
  <div class="page trace-page">
    ...
    <template v-else>
      ...
      <section class="trace-section timeline-section">
        <div class="section-heading"><span>05</span><div><h3>各环节可信时间线</h3><p>按执行时间展示每个环节的操作主体、业务结果与区块链存证</p></div></div>
        ...
        <el-timeline><el-timeline-item v-for="(e,i) in detail.evidence" :key="i" :timestamp="String(e.created_at)" placement="top" :color="chainColor(e.fabric_status)"><el-card><div class="timeline-card"><div><h4>{{ e.business_type }}</h4><p>{{ e.business_summary }}</p><span>操作人：{{ e.operator_name }} · {{ e.operator_role }} · {{ e.operator_organization }}</span>
          <div class="chain-info">
            <el-tag :type="chainStatusType(e.fabric_status)" size="mini" effect="dark">{{ chainStatusLabel(e.fabric_status) }}</el-tag>
            <span v-if="e.fabric_tx_id" class="chain-tx">TxID: {{ e.fabric_tx_id.substring(0,16) }}...</span>
            <span v-if="e.fabric_block_number" class="chain-block">区块 #{{ e.fabric_block_number }}</span>
          </div></div><el-popover trigger="click" width="520">
            <div class="chain-detail-pop">
              <div><b>事件 ID</b><code>{{ e.event_id || '-' }}</code></div>
              <div><b>数据哈希</b><code>{{ e.data_hash }}</code></div>
              <div><b>Fabric TxID</b><code>{{ e.fabric_tx_id || '尚未上链' }}</code></div>
              <div><b>区块号</b><code>{{ e.fabric_block_number || '-' }}</code></div>
              <div><b>确认时间</b><code>{{ e.confirmed_at || '尚未确认' }}</code></div>
              <div><b>上链状态</b><code>{{ chainStatusLabel(e.fabric_status) }}</code></div>
            </div>
          <el-button slot="reference" size="mini">查看链上详情</el-button></el-popover></div></el-card></el-timeline-item></el-timeline>
      </section>
    </template>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    chainStatusLabel(status) { return { confirmed: '已上链', pending: '待上链', failed: '上链失败', submitting: '提交中' }[status] || status || '待上链' },
    chainColor(status) { return { confirmed: '#318f89', pending: '#e6a23c', failed: '#f56c6c', submitting: '#909399' }[status] || '#318f89' },
